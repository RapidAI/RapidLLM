// Isolated T=4 Q6 1-row. Same dequant as gemv_t4_1row.cu. K-unroll 2 + occ 4.
...
__global__ void __launch_bounds__(256, 4) gemv_q6k_soa_f32_t4_1row_ku_k(const uint8_t* W, const float* X, float* Y,
                                                                       int m, int n, int add) {
...
void launch_q6k_f32_t4_1row_ku(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t4_1row_ku_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}
