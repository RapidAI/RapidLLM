// T=2 1-row GEMV. Isolated TU — do not edit gemv_q6_t2.cu (2-row ild stays).
// Same Q6 dequant as acc_q6_ild_t2; same Q4 dequant as acc_q4k_soa_q8_2x.
...
__global__ void __launch_bounds__(256, 3) gemv_q6k_soa_f32_t2_1row_k(const uint8_t* W, const float* X, float* Y,
                                                                    int m, int n, int add) {
...
__global__ void __launch_bounds__(256, 3) gemv_q4k_soa_q8_t2_1row_k(const uint8_t* W, const int8_t* xq,
    ...
    if (row < m && nb > 0) {
        ...
        acc_q4k_soa_q8_2x(r0, xq, xsc, xq + n, xsc + nsc, nb, lane, a0, a1);
    }
    if (row >= m) return;
    a0 = warp_sum(a0);
    a1 = warp_sum(a1);
...
void launch_q6k_f32_t2_1row(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t2_1row_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}
...
void launch_q4k_q8_t2_1row(const uint8_t* W, const int8_t* xq, const __half* xsc, float* Y, int m, int n, int add) {
    ...
    const int tw = th / 32;
    const int pb = (m + tw - 1) / tw;
    gemv_q4k_soa_q8_t2_1row_k<<<pb, th>>>(W, xq, xsc, Y, m, n, add);
}
