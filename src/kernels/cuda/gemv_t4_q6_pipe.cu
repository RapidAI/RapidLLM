// Isolated T=4 Q6 1-row with 3-stage cp.async W. Same dequant as gemv_t4_1row.cu.
...
// Same float dequant as gemv_t4_1row.cu acc_q6_ild_t4.
...
__global__ void __launch_bounds__(256, 3) gemv_q6k_soa_f32_t4_1row_pipe_k(const uint8_t* W, const float* X,
    ...
    uint8_t* wsm = smem_w + warp * (3 * kQ6KSoaBsz);
...
void launch_q6k_f32_t4_1row_pipe(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t4_1row_pipe_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}
