// Isolated T=4 Q6 1-row with split FMA chains. Same q/s/x as gemv_t4_1row.cu.
...
// Same products as acc_q6_ild_t4; two independent FMA chains per token.
__device__ __forceinline__ void acc_q6_ilp_t4(const uint8_t* blk, int lane, int is, const float* p0,
...
#pragma unroll
    for (int n128 = 0; n128 < 2; ++n128) {
        ...
        if (n128 == 0) {
            ...
            b0 = fmaf(s2, __ldg(p0 + 160 + lane), b0);
            a0 = fmaf(s3, __ldg(p0 + 192 + lane), a0);
            b0 = fmaf(s4, __ldg(p0 + 224 + lane), b0);
            a1 = fmaf(s1, __ldg(p1 + 128 + lane), a1);
            b1 = fmaf(s2, __ldg(p1 + 160 + lane), b1);
            a1 = fmaf(s3, __ldg(p1 + 192 + lane), a1);
            b1 = fmaf(s4, __ldg(p1 + 224 + lane), b1);
            a2 = fmaf(s1, __ldg(p2 + 128 + lane), a2);
            b2 = fmaf(s2, __ldg(p2 + 160 + lane), b2);
            a2 = fmaf(s3, __ldg(p2 + 192 + lane), a2);
            b2 = fmaf(s4, __ldg(p2 + 224 + lane), b2);
            a3 = fmaf(s1, __ldg(p3 + 128 + lane), a3);
            b3 = fmaf(s2, __ldg(p3 + 160 + lane), b3);
            a3 = fmaf(s3, __ldg(p3 + 192 + lane), a3);
            b3 = fmaf(s4, __ldg(p3 + 224 + lane), b3);
        }
        ql += 64;
        qh += 32;
        ds += 8;
...
__global__ void __launch_bounds__(256, 2) gemv_q6k_soa_f32_t4_1row_ilp_k(const uint8_t* W, const float* X,
    ...
    for (int b = 0; b < nb; ++b) {
        ...
        if (r0)
            acc_q6_ilp_t4(blk0, lane, is, p0, p1, p2, p3, x0a, x0b, x0c, x0d, x1a, x1b, x1c, x1d, x2a, x2b,
...
void launch_q6k_f32_t4_1row_ilp(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t4_1row_ilp_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}
