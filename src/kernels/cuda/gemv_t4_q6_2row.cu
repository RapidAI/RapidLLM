// Isolated T=4 Q6 2-row. Same dequant as gemv_t4_1row.cu; layout like T=1 2-row.
...
__global__ void __launch_bounds__(256, 2) gemv_q6k_soa_f32_t4_2row_k(const uint8_t* W, const float* X, float* Y,
    ...
    for (int b = 0; b < nb; ++b) {
        ...
        if (r0)
            acc_q6_ild_t4(s0, lane, is, p0, p1, p2, p3, x0a, x0b, x0c, x0d, x1a, x1b, x1c, x1d, x2a, x2b, x2c,
                          x2d, x3a, x3b, x3c, x3d, a00, a01, a02, a03);
        if (r1)
            acc_q6_ild_t4(s0 + kQ6KSoaBsz, lane, is, p0, p1, p2, p3, x0a, x0b, x0c, x0d, x1a, x1b, x1c, x1d,
                          x2a, x2b, x2c, x2d, x3a, x3b, x3c, x3d, a10, a11, a12, a13);
        stage ^= 1;
    }
    a00 = warp_sum(a00);
    a01 = warp_sum(a01);
...
void launch_q6k_f32_t4_2row(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t4_2row_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}
