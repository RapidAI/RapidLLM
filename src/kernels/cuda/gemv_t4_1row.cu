// T=4 1-row GEMV. Isolated TU so T=2 1-row / T=4 2-row codegen stays put.
// Same dequant as gemv_q6_t2_1row.cu (locked token-correct T=2).
...
__global__ void __launch_bounds__(256, 2) gemv_q6k_soa_f32_t4_1row_k(const uint8_t* W, const float* X, float* Y,
                                                                    int m, int n, int add) {
    ...
    for (int b = 0; b < nb; ++b) {
        ...
        if (r0)
            acc_q6_ild_t4(blk0, lane, is, p0, p1, p2, p3, x0a, x0b, x0c, x0d, x1a, x1b, x1c, x1d, x2a, x2b,
                          x2c, x2d, x3a, x3b, x3c, x3d, a0, a1, a2, a3);
        stage ^= 1;
    }
    if (row >= m) return;
    a0 = warp_sum(a0);
...
__global__ void __launch_bounds__(256, 2) gemv_q4k_soa_q8_t4_1row_k(const uint8_t* W, const int8_t* xq,
    ...
    if (row < m && nb > 0) {
        ...
        acc_q4k_soa_q8_4x(r0, xq, xsc, xq + n, xsc + nsc, xq + 2 * n, xsc + 2 * nsc, xq + 3 * n, xsc + 3 * nsc,
                          nb, lane, a0, a1, a2, a3);
    }
    if (row >= m) return;
    a0 = warp_sum(a0);
...
__global__ void __launch_bounds__(256, 2) gemv_q4k_soa_q8_t4_1row_dual_k(const uint8_t* W1, const uint8_t* W2,
    ...
    if (row < m && nb > 0) {
        const uint8_t* r0 = W1 + static_cast<size_t>(row) * static_cast<size_t>(nb) * kQ4KSoaBsz;
        const uint8_t* s0 = W2 + static_cast<size_t>(row) * static_cast<size_t>(nb) * kQ4KSoaBsz;
        acc_q4k_soa_q8_4x(r0, xq, xsc, xq + n, xsc + nsc, xq + 2 * n, xsc + 2 * nsc, xq + 3 * n, xsc + 3 * nsc,
                          nb, lane, a0, a1, a2, a3);
        acc_q4k_soa_q8_4x(s0, xq, xsc, xq + n, xsc + nsc, xq + 2 * n, xsc + 2 * nsc, xq + 3 * n, xsc + 3 * nsc,
                          nb, lane, b0, b1, b2, b3);
    }
    if (row >= m) return;
    a0 = warp_sum(a0);
...
void launch_q6k_f32_t4_1row(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t4_1row_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}
...
void launch_q4k_q8_t4_1row(const uint8_t* W, const int8_t* xq, const __half* xsc, float* Y, int m, int n,
    ...
    const int tw = th / 32;
    const int pb = (m + tw - 1) / tw;
    gemv_q4k_soa_q8_t4_1row_k<<<pb, th>>>(W, xq, xsc, Y, m, n, add);
}
...
void launch_q4k_q8_t4_1row_dual(const uint8_t* W1, const uint8_t* W2, const int8_t* xq, const __half* xsc,
    ...
    const int tw = th / 32;
    const int pb = (m + tw - 1) / tw;
    gemv_q4k_soa_q8_t4_1row_dual_k<<<pb, th>>>(W1, W2, xq, xsc, Y1, Y2, m, n);
}
