// T=4 2-row unroll. Isolated TU — locked 1-row / pipe TUs stay put.
// Each warp reduces two rows with the same 32-lane dequant as gemv_t4_1row /
// gemv_t4_pipe (not the 16+16 2-row split that drifted tokens).
...
// 28 of 32 lanes: 14×16B row0 + 14×16B row1.
__device__ __forceinline__ void cp_async_q6_pair(uint8_t* dst0, const uint8_t* src0, uint8_t* dst1,
                                                 const uint8_t* src1, int lane) {
    if (lane < 14) {
        if (src0) cp_async16(dst0 + lane * 16, src0 + lane * 16);
    } else if (lane < 28) {
        const int l = lane - 14;
        if (src1) cp_async16(dst1 + l * 16, src1 + l * 16);
    }
}

// 20 of 32 lanes: 10×16B row0 + 10×16B row1.
__device__ __forceinline__ void cp_async_q4_pair(uint8_t* dst0, const uint8_t* src0, uint8_t* dst1,
                                                 const uint8_t* src1, int lane) {
    if (lane < 10) {
        if (src0) cp_async16(dst0 + lane * 16, src0 + lane * 16);
    } else if (lane < 20) {
...
__global__ void __launch_bounds__(256, 2) gemv_q6k_soa_f32_t4_ur2_k(const uint8_t* W, const float* X, float* Y,
                                                                    int m, int n, int add) {
    ...
    auto load_stage = [&](int st, int b) {
        uint8_t* s = wsm + st * (2 * kQ6KSoaBsz);
        cp_async_q6_pair(s, r0 ? r0 + static_cast<size_t>(b) * kQ6KSoaBsz : nullptr, s + kQ6KSoaBsz,
                         r1 ? r1 + static_cast<size_t>(b) * kQ6KSoaBsz : nullptr, lane);
        cp_async_commit();
    };
    ...
    for (int b = 0; b < nb; ++b) {
        ...
        const uint8_t* s = wsm + stage * (2 * kQ6KSoaBsz);
        if (r0)
            acc_q6_ild_t4(s, lane, is, p0, p1, p2, p3, x0a, x0b, x0c, x0d, x1a, x1b, x1c, x1d, x2a, x2b, x2c,
                          x2d, x3a, x3b, x3c, x3d, a0, a1, a2, a3);
        if (r1)
            acc_q6_ild_t4(s + kQ6KSoaBsz, lane, is, p0, p1, p2, p3, x0a, x0b, x0c, x0d, x1a, x1b, x1c, x1d, x2a,
    ...
    if (lane != 0) return;
    if (row0 < m) {
        write_y(Y, row0, a0, add);
        write_y(Y + m, row0, a1, add);
        write_y(Y + 2 * m, row0, a2, add);
        write_y(Y + 3 * m, row0, a3, add);
    }
    if (row1 < m) {
        write_y(Y, row1, b0, add);
        write_y(Y + m, row1, b1, add);
        write_y(Y + 2 * m, row1, b2, add);
        write_y(Y + 3 * m, row1, b3, add);
    }
}

__global__ void __launch_bounds__(256, 3) gemv_q4k_soa_q8_t4_ur2_k(const uint8_t* W, const int8_t* xq,
    ...
    uint8_t* wsm = smem_q4 + warp * (2 * 2 * kQ4KSoaBsz);
    float a0 = 0.f, a1 = 0.f, a2 = 0.f, a3 = 0.f;
    float b0 = 0.f, b1 = 0.f, b2 = 0.f, b3 = 0.f;
    const uint8_t* r0 =
    ...
    auto load_stage = [&](int st, int b) {
        ...
        cp_async_q4_pair(s, r0 ? r0 + static_cast<size_t>(b) * kQ4KSoaBsz : nullptr, s + kQ4KSoaBsz,
                         r1 ? r1 + static_cast<size_t>(b) * kQ4KSoaBsz : nullptr, lane);
        cp_async_commit();
    };
    if (nb > 0) load_stage(0, 0);
    int stage = 0;
    for (int b = 0; b < nb; ++b) {
        ...
        if (r0)
            acc_q4k_smem_4x(s, xq, xsc, xq + n, xsc + nsc, xq + 2 * n, xsc + 2 * nsc, xq + 3 * n, xsc + 3 * nsc,
                            b, lane, a0, a1, a2, a3);
        if (r1)
...
__global__ void __launch_bounds__(256, 2) gemv_q4k_soa_q8_t4_dual_ur2_k(const uint8_t* W1, const uint8_t* W2,
    ...
    // [stage][row][W] — 2 stages × 2 rows × 2 W
    ...
    auto load_stage = [&](int st, int b) {
        ...
        // Pack 4 blocks: W1r0, W1r1, W2r0, W2r1. Two pair loads (20+20 lanes).
        cp_async_q4_pair(s, p00 ? p00 + static_cast<size_t>(b) * kQ4KSoaBsz : nullptr, s + kQ4KSoaBsz,
                         p01 ? p01 + static_cast<size_t>(b) * kQ4KSoaBsz : nullptr, lane);
        cp_async_q4_pair(s + 2 * kQ4KSoaBsz, p10 ? p10 + static_cast<size_t>(b) * kQ4KSoaBsz : nullptr,
                         s + 3 * kQ4KSoaBsz, p11 ? p11 + static_cast<size_t>(b) * kQ4KSoaBsz : nullptr, lane);
        cp_async_commit();
    };
    if (nb > 0) load_stage(0, 0);
    int stage = 0;
    ...
    if (lane != 0) return;
    if (row0 < m) {
        write_y(Y1, row0, a0, 0);
        write_y(Y1 + m, row0, a1, 0);
        write_y(Y1 + 2 * m, row0, a2, 0);
        write_y(Y1 + 3 * m, row0, a3, 0);
        if (Y2) {
            write_y(Y2, row0, c0, 0);
            write_y(Y2 + m, row0, c1, 0);
            write_y(Y2 + 2 * m, row0, c2, 0);
            write_y(Y2 + 3 * m, row0, c3, 0);
        }
    }
    if (row1 < m) {
        write_y(Y1, row1, b0, 0);
        write_y(Y1 + m, row1, b1, 0);
        write_y(Y1 + 2 * m, row1, b2, 0);
        write_y(Y1 + 3 * m, row1, b3, 0);
        if (Y2) {
            write_y(Y2, row1, d0, 0);
            write_y(Y2 + m, row1, d1, 0);
            write_y(Y2 + 2 * m, row1, d2, 0);
            write_y(Y2 + 3 * m, row1, d3, 0);
        }
    }
}
...
void launch_q6k_f32_t4_ur2(const uint8_t* W, const float* X, float* Y, int m, int n, int add) {
    ...
    gemv_q6k_soa_f32_t4_ur2_k<<<pb, th, smem>>>(W, X, Y, m, n, add);
}

void launch_q4k_q8_t4_ur2(const uint8_t* W, const int8_t* xq, const __half* xsc, float* Y, int m, int n,
                          int add) {
    ...
    gemv_q4k_soa_q8_t4_ur2_k<<<pb, th, smem>>>(W, xq, xsc, Y, m, n, add);
}

void launch_q4k_q8_t4_dual_ur2(const uint8_t* W1, const uint8_t* W2, const int8_t* xq, const __half* xsc,
                               float* Y1, float* Y2, int m, int n) {
    ...
    const int pairs = (m + 1) / 2;
    const int pb = (pairs + tw - 1) / tw;
    const size_t smem = static_cast<size_t>(tw) * 2 * 2 * 2 * kQ4KSoaBsz;
    gemv_q4k_soa_q8_t4_dual_ur2_k<<<pb, th, smem>>>(W1, W2, xq, xsc, Y1, Y2, m, n);
}
