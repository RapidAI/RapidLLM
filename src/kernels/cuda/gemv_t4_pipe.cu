// Isolated T=4 Q4 1-row / dual. cp.async W pipeline + dual interleaves W1/W2
// per K-tile so the same xq tile stays in L1. Same dequant as gemv_t4_1row.cu.
...
// Regular smem loads — __ldg/__ldcs on smem is illegal on Ada.
...
__global__ void __launch_bounds__(256, 4) gemv_q4k_soa_q8_t4_1row_pipe_k(const uint8_t* W, const int8_t* xq,
...
__global__ void __launch_bounds__(256, 3) gemv_q4k_soa_q8_t4_1row_dual_pipe_k(const uint8_t* W1, const uint8_t* W2,
...
void launch_q4k_q8_t4_1row_pipe(const uint8_t* W, const int8_t* xq, const __half* xsc, float* Y, int m, int n,
                                int add) {
    ...
    gemv_q4k_soa_q8_t4_1row_pipe_k<<<pb, th, smem>>>(W, xq, xsc, Y, m, n, add);
}

void launch_q4k_q8_t4_1row_dual_pipe(const uint8_t* W1, const uint8_t* W2, const int8_t* xq, const __half* xsc,
                                     float* Y1, float* Y2, int m, int n) {
    ...
    gemv_q4k_soa_q8_t4_1row_dual_pipe_k<<<pb, th, smem>>>(W1, W2, xq, xsc, Y1, Y2, m, n);
}
