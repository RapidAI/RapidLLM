// Isolated T=4 Q4 1-row / dual. 2-stage cp.async W (gemv_t4_pipe) + group-sx
// from ensure_xq (gemv_t4_q4_p3). Drops the per-lane sx dp4a.
...
__global__ void __launch_bounds__(256, 5) gemv_q4k_soa_q8_t4_1row_x2_k(const uint8_t* W, const int8_t* xq,
    ...
    for (int b = 0; b < nb; ++b) {
        ...
        __syncwarp();
        if (r0)
            acc_q4k_smem_4x_xsum(wsm + stage * kQ4KSoaBsz, xq, xsc, xq + n, xsc + nsc, xq + 2 * n, xsc + 2 * nsc,
...
__global__ void __launch_bounds__(256, 4) gemv_q4k_soa_q8_t4_1row_dual_x2_k(const uint8_t* W1, const uint8_t* W2,
                                                                            const int8_t* xq, const __half* xsc,
                                                                            const int32_t* xsum, float* Y1,
    ...
    for (int b = 0; b < nb; ++b) {
        ...
        if (r1)
            acc_q4k_smem_4x_xsum(s, xq, xsc, xq + n, xsc + nsc, xq + 2 * n, xsc + 2 * nsc, xq + 3 * n,
                                 xsc + 3 * nsc, xsum, xsum + nsc, xsum + 2 * nsc, xsum + 3 * nsc, b, lane, a0, a1,
                                 a2, a3);
        if (r2)
...
void launch_q4k_q8_t4_1row_x2(const uint8_t* W, const int8_t* xq, const __half* xsc, const int32_t* xsum,
                              float* Y, int m, int n, int add) {
    ...
    gemv_q4k_soa_q8_t4_1row_x2_k<<<pb, th, smem>>>(W, xq, xsc, xsum, Y, m, n, add);
}

void launch_q4k_q8_t4_1row_dual_x2(const uint8_t* W1, const uint8_t* W2, const int8_t* xq, const __half* xsc,
    ...
    gemv_q4k_soa_q8_t4_1row_dual_x2_k<<<pb, th, smem>>>(W1, W2, xq, xsc, xsum, Y1, Y2, m, n);
}
